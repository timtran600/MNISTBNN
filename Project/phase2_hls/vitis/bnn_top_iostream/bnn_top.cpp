...
)
{
    ...
    if (!load_weights)
    {
        ...
        l1_input_t input = 0;
        RECV_INPUT: for (int i = 0; i < INPUT_STREAM_WORDS; i++)
        {
            #pragma HLS PIPELINE II=1
            in_stream_t word = input_stream.read();

            // Pack 32 bits into the correct position of the 784-bit vector
            // Last word (i=24): only bits [15:0] are valid (784 - 24*32 = 16)
            int lo = i * 32;
            int hi = (i < INPUT_STREAM_WORDS - 1) ? (lo + 31) : (L1_INPUT_SIZE - 1);
            input.range(hi, lo) = word.data.range(hi - lo, 0);
        }
