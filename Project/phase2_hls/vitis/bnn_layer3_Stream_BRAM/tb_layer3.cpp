...
int main() {
    ...
    bnn_layer3_xnor(dummy_stream, dummy_input, packed_weights, 1);
    ...
    bnn_layer3_xnor(output_stream, test_input, packed_weights, 0);
    ...
    cout << "TLAST seen at neuron: " << last_seen
         << " (expected: " << LAYER3_NEURONS - 1 << ")" << endl;

    if (fail_count == 0 && last_seen == LAYER3_NEURONS - 1) {
        cout << "\nTEST PASSED" << endl;
        return 0;
    } else {
        cout << "\nTEST FAILED" << endl;
