...
typedef ap_uint<784> weight_row_t;
typedef ap_int<16> output_t;

__attribute__((sdx_kernel("bnn_layer1_xnor", 0))) void bnn_layer1_xnor(
    input_t input,
    output_t output[256],
...
{
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=bnn_layer1_xnor
# 9 "bnn_layer1.cpp"
...
#pragma HLS PIPELINE II=1
 ...
 ap_uint<784> xnor_result = ~(input ^ local_weights[n]);


            int count = 0;
            POPCOUNT_LOOP: for (int i = 0; i < 784; i++) {
                count += xnor_result[i];
            }
