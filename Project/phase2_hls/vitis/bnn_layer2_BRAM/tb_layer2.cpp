...
int main() {
    ...
    weight_row_t packed_weights[LAYER2_NEURONS];
    for (int n = 0; n < LAYER2_NEURONS; n++) {
        packed_weights[n] = 0;
        for (int i = 0; i < INPUT_SIZE; i++) {
            if (LAYER2_WEIGHTS[n][i] < 0) {
    ...
    output_t dummy_output[LAYER2_NEURONS];
    input_t dummy_input = 0;

    cout << "\nLoading weights into BRAM..." << endl;
    bnn_layer2_xnor(dummy_input, dummy_output, packed_weights, 1);
    cout << "Weights loaded." << endl;

    // ===== STEP 2: Run inference (all-zero input = all +1 bipolar) =====
    output_t test_output[LAYER2_NEURONS];
    input_t test_input = 0;

    cout << "\nRunning inference..." << endl;
    bnn_layer2_xnor(test_input, test_output, packed_weights, 0);
    ...
    cout << "\n=== RESULTS ===" << endl;
    cout << "Passed: " << pass_count << " / " << LAYER2_NEURONS << endl;
    cout << "Failed: " << fail_count << " / " << LAYER2_NEURONS << endl;
