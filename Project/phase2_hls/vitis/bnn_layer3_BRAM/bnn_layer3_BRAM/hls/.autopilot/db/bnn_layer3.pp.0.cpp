...
typedef ap_uint<256> input_t;
typedef ap_uint<256> weight_row_t;
typedef ap_int<16> output_t;

__attribute__((sdx_kernel("bnn_layer3_xnor", 0))) void bnn_layer3_xnor(
    input_t input,
    output_t output[10],
    weight_row_t weights[10],
...
{
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=bnn_layer3_xnor
# 9 "bnn_layer3.cpp"
...
#pragma HLS PIPELINE II=1
 local_weights[n] = weights[n];
    ...
    if (!load_weights) {
        NEURON_LOOP: for (int n = 0; n < 10; n++) {
#pragma HLS PIPELINE II=1


 ap_uint<256> xnor_result = ~(input ^ local_weights[n]);


            int count = 0;
            POPCOUNT_LOOP: for (int i = 0; i < 256; i++) {
                count += xnor_result[i];
            }


            output[n] = 2 * count - 256;
        }
    }
}
